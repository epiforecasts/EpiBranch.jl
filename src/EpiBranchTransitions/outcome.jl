...
`Function (ind) -> Real` — see [`Reporting`](@ref EpiBranch.EpiBranchTransitions.Reporting) for the anchor
...
`Recovery` and [`Death`](@ref EpiBranch.EpiBranchTransitions.Death) compose as competing terminal events:
...
`Death` and [`Recovery`](@ref EpiBranch.EpiBranchTransitions.Recovery) compose as competing terminal events.
"""
