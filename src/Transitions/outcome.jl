...
`Function (ind) -> Real` — see [`Reporting`](@ref EpiBranch.Transitions.Reporting) for the anchor
...
`Recovery` and [`Death`](@ref EpiBranch.Transitions.Death) compose as competing terminal events:
...
`Death` and [`Recovery`](@ref EpiBranch.Transitions.Recovery) compose as competing terminal events.
"""
