...
2. **The force of infection** `force(group, counts)` — the hazard on a
   ...
   the infectiousness-weighted number currently infectious in it: each case adds
   its own `infectiousness`, 1 by default. Homogeneous mixing is `β/N` times the
   total weighted infectious count; structured mixing applies a contact
