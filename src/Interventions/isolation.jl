# ── Isolation accessors ─────────────────────────────────────────────
# Keys owned by Isolation. Reading them is cheap dispatch on the
# `Individual.state` dict with a typed default.

"""Whether the individual is isolated. Set by `Isolation`."""
is_isolated(ind::Individual) = get(ind.state, :isolated, false)::Bool

"""Time of isolation (Float64, Inf if not isolated). Set by `Isolation`."""
isolation_time(ind::Individual) = get(ind.state, :isolation_time, Inf)::Float64

"""Whether the individual tested positive. Set by `Isolation` during
