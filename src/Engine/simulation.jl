...
(e.g. [`Reporting`](@ref EpiBranch.Transitions.Reporting), [`Hospitalisation`](@ref EpiBranch.Transitions.Hospitalisation), [`Death`](@ref EpiBranch.Transitions.Death),
[`Recovery`](@ref EpiBranch.Transitions.Recovery)) that act on each case's clinical timeline after
...
[`simulate`](@ref EpiBranch.Engine.simulate) after `initialise_state` and after each `step!` so
...
[`Isolation`](@ref) and used by [`linelist`](@ref EpiBranch.Output.linelist) to populate
`date_onset`.
...
See also [`demographics`](@ref EpiBranch.Engine.demographics), [`compose`](@ref EpiBranch.Engine.compose).
"""
...
[`compose`](@ref EpiBranch.Engine.compose) remains available as an escape hatch for cases this
builder does not cover.

See also [`clinical_presentation`](@ref EpiBranch.Engine.clinical_presentation), [`demographics`](@ref EpiBranch.Engine.demographics),
...
EpiBranch provides [`clinical_presentation`](@ref EpiBranch.Engine.clinical_presentation), [`demographics`](@ref EpiBranch.Engine.demographics),
and [`transmission_traits`](@ref EpiBranch.Engine.transmission_traits) for the common patterns; for other
...
Pass to [`simulate`](@ref EpiBranch.Engine.simulate) or [`simulate_batch`](@ref EpiBranch.Engine.simulate_batch) via the
`attributes` keyword.
"""
