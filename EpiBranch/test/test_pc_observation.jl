@testset "PerCaseObservation trait seams" begin
    ...
    @testset "scalar_detection_prob rejects non-scalar fields" begin
        ...
        @test EpiBranchObservation.scalar_detection_prob(scalar) == 0.7

        fn = PerCaseObservation(
            detection_prob = (rng, ind) -> 0.5)
        @test_throws ArgumentError EpiBranchObservation.scalar_detection_prob(fn)

        dist = PerCaseObservation(detection_prob = Beta(2.0, 2.0))
        @test_throws ArgumentError EpiBranchObservation.scalar_detection_prob(dist)
    end
