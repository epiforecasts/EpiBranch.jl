...
Only `susceptibility`, `infectiousness`, and `infection_time` are read by the engine. Everything else is owned by interventions, init functions, or the user. Interventions initialise their own fields via `initialise_individual!` and access them through accessor functions with safe defaults.
