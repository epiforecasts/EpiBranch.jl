...
- Every `RingVaccination` and `MassVaccination` parameter now accepts a `Real`, a
  ...
  `coverage` already did, and so does `GroupVaccination`'s `delay_to_immunity`.
  `delay_to_immunity` covers a vaccine whose protection
  ...
  parameter behaves exactly as before and writes none of these keys. Between two
