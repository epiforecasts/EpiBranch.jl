...
@testset "Group vaccination" begin
    ...
    @testset "A varying delay_to_immunity sets each member's immunity time" begin
        gv = GroupVaccination(efficacy = 0.9, severity_efficacy = 0.4,
            delay_to_immunity = Uniform(7.0, 21.0), dose_delay = 1.0)
        state = EpiBranch.new_state(BranchingProcess(Poisson(1.0), Exponential(5.0)),
            EpiBranch.AbstractClinicalTransition[], NoAttributes(), StableRNG(1))

        confirmed = _group_member(gv, 1, :A, test_positive = true)
        set_isolated!(confirmed, 2.0)
        members = [_group_member(gv, i, :A, test_positive = false) for i in 2:5]
        ...
        for ind in new_contacts
            ...
            @test 7.0 <= delay <= 21.0
            @test immunity_time(ind) == 3.0 + delay
            @test EpiBranch._susceptibility_risk(gv, ind).event_time == immunity_time(ind)
            @test severity_efficacy(ind) == 0.4
        end
        @test length(unique(ind.state[:vaccine_immunity_delay] for ind in new_contacts)) > 1
    end
