@testset "Generic Transition" begin
    ...
    @testset "writes :state and :state_time, anchored on :infection" begin
        ...
        state = simulate(with_attributes(bp([latent]), clinical); max_cases = 30, rng = StableRNG(1))
    ...
    @testset "chains: a state measured from an earlier transition's state" begin
        ...
        state = simulate(with_attributes(bp([severe]), clinical); max_cases = 30, rng = StableRNG(2))
        for ind in state.individuals
    ...
    @testset "probability gate" begin
        ...
        state = simulate(with_attributes(bp([never, always]), clinical); max_cases = 30, rng = StableRNG(3))
        for ind in state.individuals
    ...
    @testset "skips when the from-state was never reached" begin
        ...
        died = Transition(:died, from = :severe, delay = (rng, ind) -> 1.0,
            terminal = true)
        state = simulate(with_attributes(bp([severe, died]), clinical); max_cases = 30, rng = StableRNG(4))
        for ind in state.individuals
    ...
    @testset "terminal arbitration picks the earliest" begin
        ...
        recovered = Transition(:recovered, from = :onset, delay = (rng, ind) -> 5.0,
            terminal = true)
        state = simulate(with_attributes(bp([died, recovered]), clinical); max_cases = 30, rng = StableRNG(5))
        for ind in state.individuals
    ...
    @testset "asymptomatic case skips an onset-anchored transition" begin
        ...
        state = simulate(with_attributes(bp([sev]), all_asymp); max_cases = 30, rng = StableRNG(6))
        for ind in state.individuals
