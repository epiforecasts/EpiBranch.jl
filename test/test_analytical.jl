@testset "Analytical" begin
    ...
    @testset "Chain size likelihood" begin
        ...
        @testset "Multi-seed and right-censored chain sizes" begin
            ...
            for x in 2:8
                formula = exp(EpiBranch.EpiBranchAnalytics._chain_size_logpdf(d, x, 2))
        ...
        @testset "Real-time mixture likelihood and end_of_outbreak_probability" begin
            ...
            d = GammaBorel(0.5, 0.8)
            @test EpiBranch.EpiBranchAnalytics._chain_size_right_tail_logprob(d, 1, 1) ==
                  0.0
            @test EpiBranch.EpiBranchAnalytics._chain_size_right_tail_logprob(d, 2, 2) ==
                  0.0

            # log P(X = x) + log P(X ≥ x + 1) sums consistently with the
            # PMF: P(X = x) + P(X ≥ x + 1) = P(X ≥ x).
            for x in 2:8
                pmf = exp(EpiBranch.EpiBranchAnalytics._chain_size_logpdf(d, x, 1))
                tail_below = exp(EpiBranch.EpiBranchAnalytics._chain_size_right_tail_logprob(d, x, 1))
                tail_above = exp(EpiBranch.EpiBranchAnalytics._chain_size_right_tail_logprob(
                    d, x +
                       1, 1))
            ...
            d = GammaBorel(0.5, 0.8)
            ll_expected = sum(
                EpiBranch.EpiBranchAnalytics._chain_size_right_tail_logprob(d, x, 1)
            for x in data.data)
