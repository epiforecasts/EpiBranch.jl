...
rng = StableRNG(32)
states = simulate(model, 20; max_cases = 10, rng = rng)
sizes = Int[]
for s in states
...
rng = StableRNG(32)
states = simulate(model_fp, 20; max_cases = 10, rng = rng)
sizes = Int[]
for s in states
    cs = chain_statistics(s)
    append!(sizes, cs.size)
...
rng = StableRNG(32)
state = simulate(model; n_initial = 10, max_cases = 100, rng = rng)
...
states_nb = simulate(model_nb, 500; rng = rng)
nb_sizes = Int[]
for s in states_nb
    cs = chain_statistics(s)
