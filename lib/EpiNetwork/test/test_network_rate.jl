# Tests for NetworkRateProcess: a rate-based (contact-interval) network run
...
@testset "NetworkRateProcess" begin
    @testset "construction" begin
        ring = ring_adjacency(5)
        m = NetworkRateProcess(ring, Exponential(2.0); infectious_period = 6.0)
        @test m isa NetworkRateProcess
        ...
        ml = NetworkRateProcess(ring, Exponential(2.0);
        ...
        mp = NetworkRateProcess(ring, Exponential(2.0); progression = prog)
        @test mp.from == :infectious
        @test length(mp.progression) == 2

        # a scalar and a distribution external hazard are both accepted
        @test NetworkRateProcess(ring, Exponential(2.0); external_hazard = 0.05) isa
              NetworkRateProcess
        @test NetworkRateProcess(ring, Exponential(2.0); external_hazard = Exponential(20.0)) isa
              NetworkRateProcess
        @test_throws ArgumentError NetworkRateProcess(ring, Exponential(2.0);
            external_hazard = -1.0)
        ...
        mA = NetworkRateProcess(A, Exponential(2.0); infectious_period = 6.0)
        ...
        @test NetworkRateProcess(ring, edge_k; infectious_period = 6.0) isa
              NetworkRateProcess
        bad = [[Exponential(2.0)] for _ in ring]   # wrong per-node lengths
        @test_throws ArgumentError NetworkRateProcess(ring, bad; infectious_period = 6.0)
    end

    @testset "moderate contact rate spreads to some of a ring" begin
        n = 30
        m = NetworkRateProcess(ring_adjacency(n), Exponential(2.0);
    ...
    @testset "very short contact interval infects the whole connected graph" begin
        ...
        m = NetworkRateProcess(ring_adjacency(n), Exponential(0.05);
    ...
    @testset "isolation cuts transmission" begin
        ...
        # onward transmission — the thing the coin-flip NetworkProcess cannot
    ...
    @testset "state carries Float64 timing and renders a line list" begin
        m = NetworkRateProcess(ring_adjacency(20), Exponential(1.0);
    ...
    @testset "seeding multiple index nodes" begin
        n = 40
        m = NetworkRateProcess(ring_adjacency(n), Exponential(50.0);
