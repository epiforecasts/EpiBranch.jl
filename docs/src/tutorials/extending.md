...
competing-risks resolution decide infection. If the model's own transmission
probability is a property of the *edge* (a network's per-edge probability, a
metapopulation coupling), don't filter on it in `contacts_of` — return the
contact and contribute the probability as a competing risk by overriding
[`transmission_risks`](@ref EpiBranch.transmission_risks)`(model)` to return a
risk source with a [`competing_risk`](@ref) method. The contact is then still
produced and seen by `apply_post_transmission!` (so contact tracing and ring
vaccination work), and the probability composes with susceptibility,
