# ── Vaccination accessors ───────────────────────────────────────────
# Default-label `:vaccinated` accessor. Multi-dose schedules with custom
# `dose_label` write to `:vaccinated_<label>`; users querying those
# need to read the dict directly with the label-specific key.

"""Whether the individual is vaccinated under the default dose label.
For multi-dose vaccination, check `ind.state[:vaccinated_<label>]`
directly."""
