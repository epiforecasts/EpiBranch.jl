...
All contacts are stored, not just successful infections. This gives the **simulist**-style contacts table (with `infected` flag) and intervention effort tracking, without any additional bookkeeping.
...
Four hooks, all optional. See the [Extending guide](https://epiforecasts.github.io/EpiBranch.jl/dev/tutorials/extending) for the full input/output contract, ordering guarantees, and a `BorderClosure` worked example.
